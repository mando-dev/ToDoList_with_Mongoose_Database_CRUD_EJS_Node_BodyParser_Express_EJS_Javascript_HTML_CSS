...
const item3 = new Item ({ name: "<---hit this to delete an aitem" });   // new doc creted// specifing the values of our fields for the schema //binding the const to the model. Creating a new document from our new model 
  
const defaultItems = [item1, item2, item3];                              //putting all our items/docs from abbove into an an array. we trynna insert all these into our item collection
const listSchema = { name: String, items: [itemsSchema]};                // array of items. so we have a List Schema and Item schema. this will take in two fields. itemsSchema has been declared above                                         
const List = mongoose.model("List", listSchema);                         //mongoose model. This model was created from the listSchema right abvoe
...
app.get("/", function(req, res) {                                        // this is our root route. so this find() method triggers whenever we access home route. // we only need to load our default items once and keep it from repeating
      Item.find({},function(err, foundItems){                            //we leave {} empty since this is where the docs (default docs from above?) will be laoded. 'foundItems' is a dummy variable, it will containt all the items we found in our collection. 'Item' is ralting to our item collections. find() shoots us back an array
      if (foundItems.length === 0)  {                                    //checking to see if array is empty or not, if so thenm load up the default data
              Item.insertMany(defaultItems, function(err){               //to put something in the items collection we have to refer to the Item model
                   if(err) {console.log(err);}
                        ...
                        res.redirect("/");                               // this helps display our default data unto our root page
                        } else { res.render("list", {listTitle: "Today", newListItems: foundItems}); }//the "list" represents our list.ejs file, we are passing the 'items' array over to our ejs. The forEach/for lopp on app.js is where we are passing in these 'foundItems'. These 'foundItems' have been passed into our root route. This accepts a call back.   
...
app.get("/:customListName", function(req, res){                          //we are doing lodash to capitalize first letter. this is a user trying to access a custom list name. use express route parameters, so create a dynamic route. We should be able to print(localhost:3000/Home ) the last part of the url inside our app.get. customListName is a dummy variable that allows users to customize their lists?
      ...
      List.findOne({name: customListName}, function(err, foundList){     // the reason we are adding the 'customListName', its because its going to find a list inside the collections of list that has the name that the current user is trying to access. We are calling findOne on our list model. So the findOne() is basically looking for a match of the customListName
          ...
          if (!foundList){                                               //if foundList doesnt exist, then log that. Cheking to see if there is or inst a foundList-i guess that matches a prior list. we are only getting an object back and not an array
              ...
              name: customListName,                                     //user customizes the url, for example localhost300/whatever. Here we are gaining accesst to customListName
              items: defaultItems                                       // thest the items we created for default items. default items will always be creatd regardless if the list is custom or not
              });
              ...
              res.redirect("/" + customListName);                       //this is where the magic happens! Concatination. this is to display users their lists. 
                    } else { res.render("list", {listTitle: foundList.name, newListItems: foundList.items})}//.name and .items are tapping into the properties right above.//show an existing list since it already exists by tapping into our 'foundList'. We are taking in 2 parameters here                                     //customListName will now be the title since thats the custom title the user will be entering
                      }
                      });       
                      });       
app.post("/", function(req, res){                                       //this root route is connected to the 'form' in ejs.we are getting the "newItem" sent over from ejs form on the 'input' section of the form
    ...
    const listName = req.body.list;                                     //tapping into the listName. This is so the right item ends up on the proper list
    const item = new Item ({ name: itemName });                         //creating a new item-document based off our model in mongoDB. specifing the values of our fields for the schema. notice we created 'itemName' a few lines above.//binding the const to the model. 
...
app.post("/delete", function(req, res){                                 //this is a call back. our checkedItemId is storing the ID of item being checked off. 
    ...
    const listName = req.body.listName;                                 //here we are checking the value of our listName. Make sure that your second listName matches the name of the 2nd input in your form in ejs
          if (listName === "Today"){                                    //differentiating whether we are deleting item from detault list or custom list
              Item.findByIdAndRemove(checkedItemId, function(err){      //delete item custom list. So first we check for the 'listName' and then we find item with 'checkedItemId'. These are the 2 consts we created earlier. We have to go in the array [itemSchema] to find its particular ID and delete
              if (!err) {
...
if (port == null || port == "") {                                       // if herokus port fails then we can set up our own
    port = 3004;   
                                                  //your database is no longer being hosted locally but on mongoDB Atlas via AWS
}
app.listen(port, function() {console.log("Server has started successfully broh");});  // add this to your site as one of your projects: https://intense-sea-86040.herokuapp.com/
